...
def main() -> None:
    ...
    parser.add_argument("--industry", default="", help="Market / industry name")
    parser.add_argument(
        "--country",
        default="global",
    ...
    if args.input_json:
        ...
        query_context = {
            "industry": data.get("industry") or args.industry,
            "country": data.get("country") or args.country or "global",
            "functions": list(data.get("functions") or args.functions or []),
        }
    else:
        query_context = {
    ...
    if not str(query_context.get("industry") or "").strip():
        parser.error("Provide --industry or --input-json with an industry field")
    ...
    qpath = result.get("quadrant_path") or (result.get("coherent_quadrant") or {}).get("output_path")
    if qpath:
        print(f"  Quadrant JSON: {qpath}")
