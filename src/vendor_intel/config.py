...
class Settings(BaseSettings):
    ...
    quadrant_brand_concurrency: int = Field(
        default=6, validation_alias="QUADRANT_BRAND_CONCURRENCY"
    )
    ...
    # Google AI Overview evidence, collected by the Chrome extension in extension/.
    # Both consumers read the same on-disk cache; neither ever drives a browser.
    ai_overview_enabled: bool = Field(
        default=False, validation_alias="AI_OVERVIEW_ENABLED"
    )
    ai_overview_discovery: bool = Field(
        default=False, validation_alias="AI_OVERVIEW_DISCOVERY"
    )
    ...
    def load(cls) -> "Settings":
        ...
        if disc:
            ...
            # Env SEARXNG_BASE_URL wins (Compose sets http://searxng:8080)
            if search.get("searxng_base_url") and not os.getenv("SEARXNG_BASE_URL"):
