    ...
    *,
    ...
    for ent in entities:
        ...
        if not prev or score > prev.get("_score", 0):
            ...
            for h in hits_for[:12]:
                sn = str(getattr(h, "snippet", "") or "").strip()
                title = str(getattr(h, "title", "") or "").strip()
                url = str(getattr(h, "source_url", "") or getattr(h, "url", "") or "").strip()
    ...
    rows = [
        {
            ...
            "discovery_snippets": list(v.get("discovery_snippets") or []),
        }
