...
def _searxng_probe_targets() -> list[tuple[str, int]]:
    """Localhost + configured SEARXNG_BASE_URL host (Docker service DNS)."""
    targets: list[tuple[str, int]] = [SEARXNG_LOCAL]
    try:
        import os

        raw = (os.environ.get("SEARXNG_BASE_URL") or "").strip()
        if not raw:
            return targets
...
def probe_search_hosts() -> dict[str, bool]:
    ...
    out["searxng_local"] = any(_tcp_ok(h, p) for h, p in _searxng_probe_targets())
    return out
...
def print_connectivity_report() -> None:
    ...
    print(f"    searxng: {searx_label}", flush=True)
    if not searx_tcp:
        print(
            "  [search] Tip: run `docker compose up -d searxng` and set "
            "SEARXNG_BASE_URL=http://searxng:8080 inside Compose.",
            flush=True,
        )
