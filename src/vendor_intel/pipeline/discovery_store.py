...
Store lives at output/.discovery_store/<market_slug>__<geo_slug>.json — one per market AND
geography, so a per-country region run does not make each country inherit the previous ones. Disable with PIPELINE_DISCOVERY_STORE=0.
"""
...
def _store_path(market: str, geography: str = ""):
    """One store per (market, geography).

    Scoping by market alone made a region run compound: the USA run persisted its
    companies, then the Canada run re-injected all of them as candidates and
    re-crawled them, so country N processed roughly N times the work and Canada
    spent its budget re-judging US companies. Keying on geography keeps each
    country's cumulative coverage its own.
    """
    ...
    try:
        ...
        return None
    geo = _slug(geography) if str(geography or "").strip() else "global"
    return d / f"{_slug(market)}__{geo}.json"


_FIELDS = ("role", "value_chain_section", "company_summary", "company_function")


def load_discovered(market: str, geography: str = "") -> list[dict[str, str]]:
    ...
    p = _store_path(market, geography)
...
def save_discovered(market: str, companies: list[dict[str, Any]], geography: str = "") -> int:
    """Union newly-confirmed companies into the store (keyed by domain). Returns new store size."""
    p = _store_path(market, geography)
    if not p:
        return 0
    merged: dict[str, dict[str, str]] = {}
    for c in load_discovered(market, geography):  # same scope we are about to write
        merged[c["domain"]] = c
    ...
    try:
        ...
        tmp.write_text(
            json.dumps({"market": market, "geography": geography or "global",
                        "companies": list(merged.values())}, indent=2),
            encoding="utf-8",
        )
        tmp.replace(p)
    except Exception:
