...
def _cap(settings: Settings, name: str, default: int) -> int:
    """Resolve a row cap, preserving an explicit 0 as "uncapped".

    Plain ``getattr(...) or default`` silently turns 0 into the default, which
    made ``PIPELINE_EXPORT_MAX_ROWS=0`` fall back to 200 rows.
    """
