...
_MULTI_SYS = (
    ...
    "summary), identify EVERY section the company genuinely operates in.\n"
    "Multi-section companies are the most strategically important rows in the report, so do not "
    "under-report them. Assign a section whenever the summary shows the company performs that "
    "activity - directly, through a subsidiary, or under its own brand. These are common and "
    "SHOULD get multiple sections:\n"
    "- a manufacturer that also sells direct or runs its own distribution\n"
    "- a brand that also manufactures its products\n"
    "- a group whose divisions cover several parts of the chain\n"
    "- a supplier that also makes finished products\n"
    "Assign ONE section only when the summary genuinely shows a single activity. Never invent an "
    "activity the summary does not support. For each section give a 1-3 word role label "
    "(Manufacturer, Distributor, Brand, Supplier, Service Provider, ...). "
    "Use the EXACT section text provided.\n"
