...
async def run_pipeline(
    ...
    *,
    ...
    export_label = (
        "export uncapped (every row clearing the scope + quality gates)"
        if export_max <= 0
        else f"export up to {export_max} rows (no padding)"
    )
    print(
    ...
    # Google AI Overview mining: answers collected by the Chrome extension (extension/, drained via
    # run_ai_bridge.py) name real companies that neither search nor Wikidata surfaces — AI Overview
    # summarises across sources plain search ranks poorly. Read-only here: a miss enqueues the
    # question and this run continues, so the next run is richer. Added as DISCOVERED CANDIDATES
    # (is_seed=False) — a name in an overview is a lead, not proof, and still faces crawl +
    # relevance classification like every other channel.
    if getattr(settings, "ai_overview_discovery", False):
        try:
            ...
            # Must match what the quadrant KB uses, or the two consumers write to
            # separate cache directories and neither sees the other's answers.
            aio_market = resolve_market_key(query_context, scope)
            aio_store = AiOverviewStore(default_cache_dir(aio_market))
            geo = str(query_context.get("country") or "global")
            geo_txt = "" if geo.lower() in ("", "global") else f" in {geo}"

            # One question per declared section plus the market-wide ask, so coverage
            # follows the analyst's own value-chain definition rather than a fixed list.
            asks: list[tuple[str, str]] = [
                ("market_players", f"Which companies operate in the {aio_market}{geo_txt}?"),
                ("market_leaders", f"Who are the leading {aio_market} companies{geo_txt}?"),
            ]
            for sec in (query_context.get("sections") or [])[:12]:
                name = str(sec if isinstance(sec, str) else sec.get("name") or "").strip()
                if name:
            ...
            for purpose, text in asks:
                ...
                for entry in extract_entries(ans.markdown):
                    # "Fike Corporation & Continental Disc Corporation" is two leads
                    for nm in split_conjoined_names(entry.name):
                        if is_plausible_company_name(nm):
                            aio_names.append(nm)

            aio_n = 0
            seen_aio: set[str] = set()
            for nm in aio_names:
                nkey = re.sub(r"[^a-z0-9]", "", nm.lower())
                if not nkey or nkey in seen_aio:
                    continue
                seen_aio.add(nkey)
                # No domain yet — resolution happens downstream like other name-only leads.
                discovered_candidates.append(
                    {"name": nm, "domain": "", "company_function": "",
                     "discovery_source": "ai_overview", "is_seed": False}
                )
                aio_n += 1
            ...
            if pend:
                print(
                    ...
                    f"`python run_ai_bridge.py` + the Chrome extension, then re-run for more coverage",
                    flush=True,
                )
        except Exception as exc:
            print(f"  [pipeline] AI Overview discovery skipped: {exc}", flush=True)
    ...
    if not recall:
        try:
            ...
            if _store_on():
                ...
                print(f"  [pipeline] discovery store: market={store_market!r} geo={country!r} -> loaded {len(stored)} "
    ...
    async def _classify_one(c: dict[str, str]) -> dict[str, Any]:
        ...
        # Inline KB for Coherent Quadrant — crawl/SSC + discovery snippets + classify.
        # Quadrant stage never re-searches the web.
        from vendor_intel.quadrant.snapshot import build_evidence_snapshot

        verdict["evidence_snapshot"] = build_evidence_snapshot(
    ...
    if recall:
        ...
        # Cross-row pass: every gate above judges one company at a time, so an SEO
        # doorway network (one operator, fifty per-country domains) passes all of
        # them. 53 of 191 rows on a real North America run were exactly that.
        try:
            ...
            final, _doorways = filter_doorways(final)
            final, _variants = dedupe_name_variants(final)
            export_rejected.extend(_doorways)
            export_rejected.extend(_variants)
            if _doorways:
                print(
                    f"  [pipeline] doorway networks collapsed: {len(_doorways)} row(s) "
                    f"(one canonical row kept per network)",
                    flush=True,
                )
            if _variants:
                print(f"  [pipeline] name-variant duplicates: {len(_variants)} row(s)", flush=True)
        except Exception as exc:
            print(f"  [pipeline] doorway filter skipped: {exc}", flush=True)
        if export_rejected:
            reasons = Counter(
    ...
    if getattr(settings, "quadrant_enabled", True) and final:
        try:
            ...
            print(f"  [pipeline] coherent quadrant skipped: {exc}", flush=True)
            coherent_quadrant = {"error": str(exc)}
    ...
    return {
        ...
        "coherent_quadrant": coherent_quadrant,
        "quadrant_path": coherent_quadrant.get("output_path") if coherent_quadrant else None,
    }
...
def save_pipeline_csv(result: dict[str, Any], path: str) -> str:
    ...
    if multi_rows:
        # Companies spanning two or more parts of the value chain (a brand that
        # also manufactures) are the most strategically significant rows in the
        # report, so they lead it rather than trailing the single-role sections.
        grouped = [("Strategic Multi-Segment Players", multi_rows)] + grouped
