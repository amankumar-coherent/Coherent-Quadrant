...
def filter_for_export(
    ...
    *,
    ...
    if max_rows <= 0:
        # Uncapped: every row that cleared the relevance + quality gates ships.
        # The gates above are the only thing standing between scope and noise now,
        # so min_quality does the work the row cap used to do implicitly.
        export.extend(r for _, r in scored)
    else:
