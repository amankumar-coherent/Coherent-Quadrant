...
def _print_result(query: str, country: str, result: dict, elapsed: float) -> None:
    ...
    if result.get("quadrant_path") or (result.get("coherent_quadrant") or {}).get("output_path"):
        print(
            f"  Quadrant JSON      : "
            f"{result.get('quadrant_path') or (result.get('coherent_quadrant') or {}).get('output_path')}"
        )
    print(f"{bar}\n")
