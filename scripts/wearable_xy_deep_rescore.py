#!/usr/bin/env python3
"""
Deep-crawl + evidence-only X/Y/Overall rescore for Wearable Medical Devices.

Why previous scores looked low:
  Last pass used thin DDGS snippets and skipped site crawl (QUAD_SKIP_FETCH=1).
  Evidence-only scoring correctly stays mid/low when KB is thin.

This pass:
  1) smart_crawl deep site crawl → evidence_snapshot
  2) DDGS web search snippets (supplement)
  3) DeepSeek scores ONLY from that evidence (no invented products/certs/revenue)
  4) Overall=(X+Y)/2; half-median quadrants
...
Env:
  ...
  QUAD_SKIP_CRAWL=1          (DDGS only — not recommended)
  QUAD_SKIP_DDGS=1
  QUAD_RESCORE_RESUME=1      keep already-scored rows from prior deep pass
"""
...
Use ONLY the EVIDENCE text provided. Do NOT invent products, FDA/CE clearances, revenue,
partnerships, countries, hospital names, or clinical claims absent from the evidence.
...
If evidence is silent on a feature → score 3–5 and grounding=insufficient for that feature.
When crawl/search evidence CLEARLY supports strength (FDA portfolio, global markets,
hospital adoption, revenue scale), DO NOT compress to mid-band — use 8–10.
...
All five X features and five Y features required. Never use grounding=model_knowledge."""
...
def _axis_100(feat_scores: list[dict], features: list[str], weights: list[float]) -> int:
    ...
    for feat, w in zip(features, weights):
        ...
        # Only stretch when evidence actually supports — never for insufficient
        if g in {"supported", "partial"} and 5 <= s <= 8:
            s = min(10.0, s + 1.0)
        if g == "insufficient":
            s = min(s, 5.0)
        total += w * (s / 10.0)
...
def _web_search(brand: str, website: str, specialty: str) -> str:
    ...
    queries = [
        f"{brand} wearable medical device FDA OR CE OR clinical trial",
        f"{brand} CGM OR ECG OR hearing aid OR continuous monitoring market share",
        f"{brand} revenue OR partnership OR hospital OR distribution countries",
        f"{brand} product portfolio medical wearable",
    ]
    if specialty:
        queries.append(f"{brand} {specialty[:80]}")
    dom = _domain(website)
    if dom:
        queries.append(f"site:{dom} {brand} FDA OR product OR clinical")
...
def _snap_text(snap: Any) -> str:
    ...
    for key in ("company", "business", "financials", "relationships", "intel", "location"):
        ...
        if isinstance(block, dict) and block:
            parts.append(f"[{key}]\n{json.dumps(block, ensure_ascii=False)[:3500]}")
...
Score now. JSON only."""
    parsed = llm_mod.llm_complete_json(SYSTEM, user, max_tokens=2200)
...
def _load_rows(*, resume: bool) -> list[dict]:
    ...
    for r in rows_raw[1:]:
        ...
        prior_x = str(d.get("X Score") or "").strip()
        prior_y = str(d.get("Y Score") or "").strip()
        prior_q = str(d.get("Quadrant") or "").strip()
        prior_o = str(d.get("Overall Score") or "").strip()
        if det:
            if det.get("Brand"):
            ...
            if resume:
                if not prior_x and det.get("X") not in (None, ""):
                    prior_x = str(det.get("X"))
                if not prior_y and det.get("Y") not in (None, ""):
                    prior_y = str(det.get("Y"))
                if not prior_q and det.get("Quadrant"):
                    prior_q = str(det.get("Quadrant"))
                if not prior_o and det.get("Overall") not in (None, ""):
                    prior_o = str(det.get("Overall"))
        d["Brand"] = brand
        d["Company"] = brand
        ...
        if resume and prior_x and prior_y and deep_done:
            d["X Score"] = prior_x
            d["Y Score"] = prior_y
            d["Overall Score"] = prior_o or str(round((float(prior_x) + float(prior_y)) / 2))
            d["Quadrant"] = prior_q
...
def _save(kept: list[dict], *, partial: bool = False, meta: dict | None = None) -> None:
    ...
    for det in detail_rows:
        ...
        # Preserve verified Found in
        fi = str(src.get("Found in") or src.get("Headquarters") or "").strip()
        if fi:
            det["Found in"] = fi

    # Merge any on-disk Found in verification so mid-run saves don't wipe HQ
    fi_path = OUT / "_audit" / "wearable_found_in_verification.json"
    if fi_path.exists():
        try:
            fi_data = json.loads(fi_path.read_text(encoding="utf-8"))
            by_fi = {
                _norm(r.get("brand") or ""): str(r.get("found_in") or "").strip()
                for r in (fi_data.get("rows") or [])
                if r.get("found_in")
            }
            for row in kept:
                key = _norm(row.get("Brand") or "")
                if key in by_fi:
                    row["Found in"] = by_fi[key]
                    row["Headquarters"] = by_fi[key]
            for det in detail_rows:
                key = _norm(det.get("Brand") or "")
                if key in by_fi:
                    det["Found in"] = by_fi[key]
        except Exception:
            pass
...
async def _kb_evidence(row: dict) -> str:
    """Build KB text from deep-crawl snapshot (preferred) + landscape fields."""
...
async def _score_one(row: dict, sem: asyncio.Semaphore) -> dict[str, Any]:
    ...
    async with sem:
        ...
        evid_chars = len(crawl_txt) + len(search_txt)
        if evid_chars < 120:
            return {
                "brand": brand,
                "ok": False,
...
async def main() -> int:
    ...
    if limit > 0:
        # score only first N unscored (or first N overall if not resume)
    ...
    print(
        f"Deep-crawl evidence rescore {len(need)}/{len(kept)} "
    ...
    settings = Settings.load()
    # Force deeper crawl page budget for this run
    try:
    ...
    for start in range(0, len(need), batch_n):
        ...
        _save(
            ...
            meta={"scored": sum(1 for r in kept if str(r.get('X Score') or '').strip())},
        )
        path = OUT / "_audit" / "wearable_xy_deep_rescore.json"
        path.write_text(
            json.dumps(
                {"scored": len(audit_log), "rows": audit_log, "method": "deep_crawl"},
                indent=2,
                ensure_ascii=False,
            ),
            encoding="utf-8",
    ...
    md = OUT / "_audit" / "wearable_xy_deep_rescore.md"
    lines = [
        "# Deep-crawl X/Y/Overall rescore — Wearable Medical Devices",
        "",
        "Previous thin DDGS-only pass underscored brands because evidence was thin.",
        "This pass uses **smart_crawl deep site crawl + DDGS + DeepSeek evidence-only**.",
        "",
        f"Scored this run: **{len(audit_log)}** · OK **{ok_n}** · crawled **{crawl_ok}**",
        f"Landscape complete: **{all_scored}** ({sum(1 for r in kept if str(r.get('X Score') or '').strip())}/{len(kept)})",
        "",
        ...
        "## Sample (this run)",
        "",
        "| Brand | X | Y | Overall | Crawled | OK |",
        "|---|---:|---:|---:|:---:|---|",
    ]
    for a in sorted(audit_log, key=lambda z: -((z.get("x") or 0) + (z.get("y") or 0)))[:40]:
        o = round(((a.get("x") or 0) + (a.get("y") or 0)) / 2)
        lines.append(
            f"| {a['brand']} | {a.get('x')} | {a.get('y')} | {o} | "
            f"{a.get('crawled')} | {a.get('ok')} |"
        )
