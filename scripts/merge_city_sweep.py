#!/usr/bin/env python3
"""Merge city_sweep_shard*.json into the checkpoint's `recalled` list.

Run once every parallel_city_sweep.py shard has finished. Dedupes on
company name (case-insensitive) against what is already in the checkpoint.

    .\\.venv\\Scripts\\python.exe scripts\\merge_city_sweep.py ^
...
def main() -> int:
    ...
    shards = sorted(out_dir.glob("city_sweep_shard*.json"))
    if not shards:
    ...
    for path in shards:
        ...
        for row in rows:
            ...
            if not key or key in seen:
                continue
            seen.add(key)
            row.pop("_swept_city", None)
            existing.append(row)
            added += 1
        n_cities = len({r.get("_swept_city") for r in rows})
        print(f"  {path.name}: {n_cities} cities swept, "
